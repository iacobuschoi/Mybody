...
console.log('\n[4-2-2] 받아오기가 한 번 실패한 것은 "껐다" 가 아니다');
/* sync.pull() 은 스냅샷 요청이 실패하면 rows:[] 로 넘깁니다. 그걸 껐다고
   읽으면 네트워크가 한 번 깜빡인 것만으로 소식이 통째로 지워지고
   기준선까지 사라져서, 그 다음 진짜 운동이 조용히 넘어갔습니다. */
reset();
N.apply(snap('f1', 1), FR, '2026-09-20T10:00:00Z');
N.apply(snap('f1', 2), FR, '2026-09-20T11:00:00Z');
N.apply([{ id: 'f1', rows: [], allowed: {} }], FR, '2026-09-20T12:00:00Z');
t('못 받아온 것으로는 소식이 안 지워진다', N.list().length === 1, N.list());
t('기준선도 안 날아간다 (다음 증가가 소식이 된다)',
  N.apply(snap('f1', 3), FR, '2026-09-20T13:00:00Z') === 1);
