...
const CHECKS = [
  ...
  { id: '앱 알림(FCM)', level: 'BLOCK', slow: false, cmd: ['node', 'tools/test-fcm.js'],
    why: '공유 설정을 끈 친구에게 앱 알림이 가면 유출입니다 — 구글 · 애플이 읽는 평문에 이름 · 숫자가 실려도 그렇습니다' },
  { id: '안드로이드 설정', level: 'BLOCK', slow: false, cmd: ['node', 'tools/test-android.js'],
    why: '앱 알림 설정 파일이 없는 날의 빌드가 깨지거나, 알림이 「기타」 채널로 새면 폰에서야 압니다' },
  ...
  { id: '계정 삭제 페이지', level: 'BLOCK', slow: true,
    cmd: ['node', 'tools/test-delete-page.js'],
    why: '스토어에 "여기서 계정을 지울 수 있다" 고 적어 내는 주소입니다 — 적어 낸 주소가 실제로 안 지우면 그건 거짓 신고입니다' },
  { id: '친구 요청 알림', level: 'BLOCK', slow: true,
    cmd: ['node', 'tools/test-friendpush.js'],
    why: '알림이 엉뚱한 사람에게 가면 그건 기능이 아니라 유출입니다 — 실제로 날아온 암호문을 풀어서 누구에게 무엇이 갔는지 봅니다' },
  ...
  { id: '지운 것이 지워지는가', level: 'BLOCK', slow: false,
    cmd: ['node', 'tools/test-forget.js'],
    why: '탈퇴하면 서버에 판독 횟수가 안 남는가. 상대가 탈퇴하면 내 기기에서도 ' +
         '그 사람의 이름과 얼굴 사진이 사라지는가. 못 지킨 약속은 안 한 약속보다 나쁩니다' },
