 ...
 *           그 변화량, 이번 주에 기록했는지 여부, 그리고 이번 주 운동 일정의
 *           **숫자 네 개**(계획한 날 · 지킨 날 · 체크 없는 날 · 남은 날).
 *           로그인했을 때만. 친구가 실제로 볼 수 있는 것은 그 친구에게
 *           켜 둔 항목뿐이지만, 값 자체는 서버에 올라가 있다.
 *           프로필 사진도 나간다 — 이건 여기가 아니라 계정에 붙어 있다
 *           (server/db.js users.avatar, 수락한 친구에게만).
 *   안 나감 측정 기록 · 프로필 · 목표 · 계획 · 체크인 · 식단 기록,
 *           그리고 **무슨 요일에 무슨 운동을 했는지**. 일정은 숫자로만
 *           나가고 요일과 종목은 스냅샷에 담기지도 않는다 — 요일이 나가면
 *           친구가 남의 한 주를 재구성할 수 있다.
...
(function (global) {
   ...
   *
   * new Date('2026-09-14') 는 **UTC 자정**으로 읽힙니다. 한국(UTC+9)에서는
   * 그게 같은 날 오전 9시라 티가 안 나는데, 미국 서부(UTC-7)에서는
   * 전날 오후 5시가 됩니다 — getDate() 가 13 을 돌려줍니다.
   *
   * 그래서 날짜 키를 다시 dayKey() 에 넣으면 하루씩 밀렸습니다. 쓸 때와
   * 읽을 때가 똑같이 밀리므로 화면은 멀쩡해 보이는데, 스트릭은 밀린 키와
   * 안 밀린 오늘(인자 없는 dayKey())을 견주게 돼서 오늘 것을 영영 못
   * 찾습니다. 시간대가 UTC 이거나 한국이면 시험도 전부 통과합니다.
   * (TZ=America/Los_Angeles 로 돌리면 42개 중 7개가 깨졌습니다.)
   *
   * 시각이 붙은 ISO 문자열('...T10:00:00Z')은 진짜 순간이므로 지금처럼
   * 현지 시각으로 바꿉니다 — 그건 밀리는 게 아니라 맞는 변환입니다. */
  function dayKey(d) {
  ...
  function weeklySnapshot() {
     ...
     * 운동한 사실이 없어지지는 않습니다.
     *
     * 예전엔 주간 체크인(P08) 하나만 봤습니다("한 번이라도 기록했는가"
     * 에서 "이번 주에" 로 고친 적은 있지만 재료는 그대로였습니다).
     * 그런데 이제 운동을 한 뒤 홈에서 칸을 체크하는 길이 생겼고, 그쪽만
     * 쓰는 사람이 친구 화면에서는 영원히 "이번 주 아직" 으로 남았습니다 —
     * 매일 체크하고 있는데요. 재는 것이 "기록을 했는가" 라면 둘 다
     * 기록입니다.
     *
     * 계획만 세운 주는 안 칩니다. 하기로 한 것은 기록이 아닙니다. */
    var wk = weekStartOf();
  ...
  function publishWeekly() {
    ...
    /* 아직 온보딩도 안 끝났는데 측정이 0건이면 안 올립니다.
     *
     * 이게 원래 사건의 모양입니다 — 앱을 지웠다 다시 깔고 로그인하면
     * 온보딩의 첫 저장이 publishWeekly() 를 부르고, 그 빈 값이 서버에
     * 멀쩡히 남아 있던 이번 주 기록을 덮어썼습니다.
     *
     * 다만 이제 "측정 0건" 하나로는 막지 않습니다. 인바디를 한 번도
     * 안 올린 사람도 운동 일정은 쓰고, 그 사람의 친구는 그걸 볼 수
     * 있어야 하니까요. 그래서 "설치 직후" 라는 조건을 같이 봅니다.
     *
     * 남는 구멍 하나는 알고 둡니다: 다시 깔고 → 온보딩을 끝내고 →
     * 백업을 불러오기 전에 일정을 정하면, 그 주의 몸 숫자가 친구
     * 화면에서 잠깐 사라집니다. 다음 측정을 넣으면 그대로 돌아옵니다.
     * 좁고 일시적이고 저절로 낫는 문제와, 인바디 없는 사람의 친구
     * 화면이 영원히 비는 문제 중에 앞쪽을 골랐습니다. */
    if (!state.onboarded && !(state.scans || []).length) {
