    ...
    render: function (wrap, ctx) {
      ...
      function honestyCard() {
        ...
        card.appendChild(h('p.muted', {
          text: '체중은 수분·나트륨·글리코겐 때문에 하루 사이에도 ±1kg 흔들립니다. ' +
                '전날 짜게 먹었거나, 탄수화물을 몰아 먹었거나, 잠이 모자랐으면 ' +
                '체지방이 전혀 늘지 않아도 숫자는 올라갑니다. ' +
                '그래서 한 번의 측정이 아니라 주 평균의 방향으로 판단해야 합니다.'
        }));
        card.appendChild(h('p.muted', {
          text: '판단 기준: 이번 주 평균과 지난 주 평균을 비교하고, ' +
                '2주 연속 같은 방향으로 벗어날 때만 계획을 건드립니다. ' +
                '체지방·골격근이 실제로 어떻게 움직였는지는 인바디로만 확인됩니다.'
        }));
