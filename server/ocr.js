...
/* 어떤 모델로 읽을 것인가.
 *
 * 기본은 Sonnet 5 입니다. 처음에는 Opus 5 였습니다 — "결과지 한 장이
 * 몇 달치 기록의 출발점이니 제일 잘 읽는 것" 이라는 이유였고, 그 자체는
 * 지금도 맞습니다. 바꾼 이유는 값이 아니라 **그물이 세 겹이기 때문**입니다:
 *   1. 도구 스키마가 14칸을 타입까지 정해서 받습니다 (산문이 올 자리 없음)
 *   2. crosscheck.js 가 결과지 내부 수식으로 검산합니다 — 한 칸을 잘못
 *      읽으면 다른 칸과 어긋나서 "모순" 으로 잡힙니다
 *   3. 사람이 검수 화면에서 확정합니다
 * 오독이 조용히 저장되는 길이 막혀 있으니, 여기서 5배를 낼 이유가 없습니다.
 *
 * 자기 결과지로 직접 재 보고 정하는 것이 제일 낫습니다:
 *   node tools/ocr-compare.js 결과지.jpg --runs=3
 *   node tools/serve.js --setup --model=claude-opus-5
 *
 * 한 장(1131×1600 결과지)에 드는 값은 대략 이렇습니다 — 입력 4천 토큰 ·
 * 출력 250 토큰 기준, 2026-09-21 공시가:
 *   claude-opus-5    $5/$25 per M  →  약 $0.027  (37원쯤)
 *   claude-sonnet-5  $2/$10 per M  →  약 $0.011  (15원쯤)  ← 기본
 *   claude-haiku-4-5 $1/$5  per M  →  약 $0.005  (7원쯤)
 * 환율은 대략값입니다. 서버는 하루 횟수(OCR_PER_DAY)로 한 번 더 막습니다. */
const DEFAULT_MODEL = process.env.OCR_MODEL || 'claude-sonnet-5';
